...
(define (function l)
  (cond ((null? l) '())
        (else
            (let ((l1 (algo:dictionary-closure (car l))))
    (cond ((equal? l1 #f) (function (cdr l)))
        (else  (utils:show-key (algo:secret-word-enumeration l1))
           (append (list (algo:secret-word-enumeration l1)) (function (cdr l)))))))))
        
  (function (map (lambda (x) (utils:add-substitution x (make-list 26 #\_))) strategies)))

  
  ;(flatten (map (lambda (x) (if (algo:dictionary-closure x) (algo:secret-word-enumeration (algo:dictionary-closure x)) '() ))
   ;     (map (lambda (x) (utils:add-substitution x (make-list 26 #\_))) strategies))))

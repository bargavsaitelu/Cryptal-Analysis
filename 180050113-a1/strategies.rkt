...
(define (etai key)
  (define (sorter l1 l2)
  (cond ((null? l1) '())
        ( (equal? (remove (car l1) l2) l2) (sorter (cdr l1) l2))
          (else (append (list (car l1)) (sorter (cdr l1)  l2)))))
  (let* ((mono (stats:cipher-monograms utils:ciphertext))
       (single1 (stats:cipher-common-words-single  utils:cipher-word-list))
       (single (list (car (string->list (car single1))) (car (string->list (cadr single1)))))
       (bigram (stats:cipher-bigrams utils:cipher-word-list))
       (nbd (stats:cipher-unique-neighbourhood bigram 'both ))
       (f5 (list (list-ref mono 0) (list-ref mono 1) (list-ref mono 2) (list-ref mono 3) (list-ref mono 4)))
       (f3 (remove (car single) (remove (cadr single) f5)))
       (nbdcar (map (lambda (x) (car x)) nbd))
       (sortf3 (sorter nbdcar f3)))
    
  (define (shuffle1 l)
    (append (list (cons (caar l) (cdadr l)) (cons (caadr l) (cdar l))) (cddr l)))
   (define (shuffle2 l)
     (reverse (shuffle1 (reverse l))))
   (define (subs a b)
     (append  (list (list (cons #\E a) (cons #\T b) (cons #\A (car single)) (cons #\I (cadr single)))
               (shuffle2 (list (cons #\E a) (cons #\T b) (cons #\A (car single)) (cons #\I (cadr single)))))))
    (define (subs1 a b)
      (append (subs a b) (subs b a)))
    
   (define (substitutions key)
     (cond ((= (length f3) 3)
            (let ((sf3 sortf3))
                  
     
     (append (subs1 (car sf3) (caddr sf3))   (subs1 (cadr sf3) (car sf3))
             (subs1 (caddr sf3) (cadr sf3)) )))
           ((= (length f3) 4)
            (let ((sf4 sortf3))
      (append (subs1 (car sf4) (car (reverse sf4))) (subs1 (cadr sf4) (cadr (reverse sf4)))
              (subs1 (cadr (reverse sf4)) (car sf4)) (subs1 (car (reverse sf4)) (cadr sf4))
              (subs1 (cadr sf4) (car sf4)) (subs1 (cadr sf4) (car (reverse sf4))))))))
